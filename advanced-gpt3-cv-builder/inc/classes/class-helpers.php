 ...
 */
 class Helpers {
	...
	protected function setup_hooks() {
		if( ! defined( 'ADVANCEDGPT3CVBUILDER_PROJECT_OPTIONS' ) ) {define( 'ADVANCEDGPT3CVBUILDER_PROJECT_OPTIONS', (array) get_option( 'advanced-gpt3-cv-builder', [] ) );}
		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/system/getoption', [ $this, 'getOption' ], 10, 2 );
		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/system/isactive', [ $this, 'isActive' ], 10, 1 );

		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/database/countries', [ $this, 'databaseCountries' ], 10, 2 );
		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/database/countryflags', [ $this, 'countryFlags' ], 10, 1 );

		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/user/dashboardpermalink', [ $this, 'dashboardPermalink' ], 10, 2 );
		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/user/visitorip', [ $this, 'visitorIP' ], 10, 0 );


		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/filter/server/time', [ $this, 'serverTime' ], 10, 2 );
		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/filter/string/random', [ $this, 'generateRandomString' ], 10, 2 );

		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/filesystem/filemtime', [ $this, 'filemtime' ], 10, 2 );
		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/filesystem/uploaddir', [ $this, 'uploadDir' ], 10, 2 );
		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/mailsystem/sendmail', [ $this, 'sendMail' ], 10, 1 );

		add_filter( 'futurewordpress/project/advancedgpt3cvbuilder/notices/manager', [ $this, 'noticeManager' ], 10, 3 );
		
		add_action( 'wp_ajax_futurewordpress/project/advancedgpt3cvbuilder/filesystem/upload', [ $this, 'uploadFile' ], 10, 0 );
		add_action( 'wp_ajax_nopriv_futurewordpress/project/advancedgpt3cvbuilder/filesystem/upload', [ $this, 'uploadFile' ], 10, 0 );
		add_action( 'wp_ajax_futurewordpress/project/advancedgpt3cvbuilder/filesystem/remove', [ $this, 'removeFile' ], 10, 0 );
		add_action( 'wp_ajax_nopriv_futurewordpress/project/advancedgpt3cvbuilder/filesystem/remove', [ $this, 'removeFile' ], 10, 0 );
		add_action( 'admin_post_futurewordpress/project/advancedgpt3cvbuilder/filesystem/download', [ $this, 'downloadFile' ], 10, 0 );
		add_action( 'admin_post_nopriv_futurewordpress/project/advancedgpt3cvbuilder/filesystem/download', [ $this, 'downloadFile' ], 10, 0 );
    ...
    return isset( ADVANCEDGPT3CVBUILDER_PROJECT_OPTIONS[ $option ] ) ? ADVANCEDGPT3CVBUILDER_PROJECT_OPTIONS[ $option ] : $default;
	}
	/**
    ...
    return ( isset( ADVANCEDGPT3CVBUILDER_PROJECT_OPTIONS[ $option ] ) && ADVANCEDGPT3CVBUILDER_PROJECT_OPTIONS[ $option ] == 'on' );
	}
	/**
	...
	public function sendMail( $args = [] ) {
		...
		// $msg = [ 'status' => 'success', 'message' => __( get_FwpOption( 'msg_profile_edit_success_txt', 'Changes saved' ), ADVANCEDGPT3CVBUILDER_PROJECT_TEXT_DOMAIN ) ];
	...
	public function uploadDir( $file = false, $force = false ) {
		...
		if( $this->theUploadDir === false ) {
			...
			$uploadDir[ 'basedir' ] = $uploadDir[ 'basedir' ] . '/' . apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/system/getoption', 'ftp-mediadir', 'futurewordpress' );
			$uploadDir[ 'baseurl' ] = $uploadDir[ 'baseurl' ] . '/' . apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/system/getoption', 'ftp-mediadir', 'futurewordpress' );
	...
	public function uploadFile() {
		...
		if( isset( $_FILES[ 'blobFile' ] ) || isset( $_FILES[ 'file' ] ) ) {
			...
			// 	$error = sprintf( __( 'File is larger then allowed range. (%d)', 'advanced-gpt3-cv-builder' ), $file[ 'size' ][0] );
			// }
			...
			// 	$error = sprintf( __( 'File format (%s) is not allowed.', 'advanced-gpt3-cv-builder' ), $extension );
			// }
			...
			if( $error === false && move_uploaded_file( $file[ 'tmp_name' ][0], $file[ 'full_path' ] ) ) {
				...
				// $meta['type'] = apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/validate/format', $meta['type'], $meta );
				$oldMeta[] = $meta;
				WC()->session->set( 'uploaded_files_to_archive', $oldMeta );
				wp_send_json_success( [
					'message'			=> __( 'Uploaded successfully', 'advanced-gpt3-cv-builder' ),
					'dropZone'		=> $meta
				], 200 );
			} else {
				$error = ( $error ) ? $error : __( 'Something went wrong while tring to upload short clip video.', 'advanced-gpt3-cv-builder' );
				wp_send_json_error( $error, 200 );
			}
		}
		wp_send_json_error( __( 'Error happens.', 'advanced-gpt3-cv-builder' ) );
	}
	public function removeFile() {
		...
		if( isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			wp_send_json_success( __( 'File removed from server.', 'advanced-gpt3-cv-builder' ), 200 );
		} else {
			wp_send_json_error( __( 'Failed to delete. Maybe File not found on server or your request doesn\'t contain file data enough.', 'advanced-gpt3-cv-builder' ), 200 );
		}
	}
	public function downloadFile() {
		...
		if( isset( $fileInfo[ 'full_url' ] ) && isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			print_r( $fileInfo );
			wp_die( __( 'File not found', 'advanced-gpt3-cv-builder' ), __( '404 not found', 'advanced-gpt3-cv-builder' ) );
		}
	}
	...
	public function dashboardPermalink( $id, $user = 'me' ) {
		if( ! defined( 'ADVANCEDGPT3CVBUILDER_PROJECT_DASHBOARDPERMALINK' ) ) {
			...
			define( 'ADVANCEDGPT3CVBUILDER_PROJECT_DASHBOARDPERMALINK', $dashboard_permalink );
		}
		$profile = ( apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/system/getoption', 'permalink-userby', 'id' ) == 'id' ) ? ADVANCEDGPT3CVBUILDER_PROJECT_DASHBOARDPERMALINK . '/' . ( ( $id ) ? $id : 'me' ) : ADVANCEDGPT3CVBUILDER_PROJECT_DASHBOARDPERMALINK . '/' . $user;
		return $profile . '/' . apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/profile/defaulttab', 'profile' );
	}
