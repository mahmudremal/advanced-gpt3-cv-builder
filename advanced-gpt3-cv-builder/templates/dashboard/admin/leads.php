        ...
        <div class="card-body">
          <div class="fancy-table table-responsive border rounded">
            <table class="table table-striped mb-0">
              <thead>
                <tr>
                  <th scope="col"><?php esc_html_e( 'Email', 'advanced-gpt3-cv-builder' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Profiles', 'advanced-gpt3-cv-builder' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Client Status', 'advanced-gpt3-cv-builder' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Meeting Time & link', 'advanced-gpt3-cv-builder' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Retainer Amount', 'advanced-gpt3-cv-builder' ); ?></th>
                  <!-- Contract Options -->
                  <th scope="col"><?php esc_html_e( 'Status', 'advanced-gpt3-cv-builder' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Action', 'advanced-gpt3-cv-builder' ); ?></th>
                </tr>
              </thead>
              <tbody>
                <?php if( count( $subscribers ) <= 0 ) : ?>
                  <tr>
                    <td colspan="7"><img src="<?php echo esc_url( ADVANCEDGPT3CVBUILDER_PROJECT_BUILD_URI . '/icons/not-found.svg' ); ?>" alt="<?php esc_attr_e( 'Noting found', 'advanced-gpt3-cv-builder' ); ?>" style="width: 100%;height: auto;max-height: 400px;"></td>
                  </tr>
                <?php endif; ?>
                <?php foreach( $subscribers as $subscriber ) :
                  ...
                  $userInfo = (object) wp_parse_args( $userInfo, [
                    'id'      => '',
                    'meta'      => (object) wp_parse_args( $userMeta, apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/usermeta/defaults', (array) $userMeta ) )
                  ] );
                  $activeSubscription = in_array( $userInfo->meta->enable_subscription, [ 'on' ] );
                  $doc = apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/esign/userdocument', false, $userInfo );$doc = ( $doc && is_array( $doc ) ) ? (object) $doc: $doc;
                  if( $doc && ! empty( $doc->document_status ) ) {$is_signed = ( $doc->document_status == 'signed' );} else {$is_signed = null;}
                    //   Missing
                  $is_signed = ( $is_signed === null ) ? __( 'Pending', 'advanced-gpt3-cv-builder' ) : (
                    ( $is_signed === true ) ? __( 'Signed', 'advanced-gpt3-cv-builder' ) : __( 'Pending', 'advanced-gpt3-cv-builder' )
                  );
                  $is_active = $activeSubscription ? __( 'Active', 'advanced-gpt3-cv-builder' ) : __( 'Paused', 'advanced-gpt3-cv-builder' );
                  ...
                  <tr id="lead-<?php echo esc_html( $userInfo->ID ); ?>">
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        ...
                        <?php $flag = apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/database/countryflags', get_user_meta( $userInfo->ID, 'country', true ) );if( $flag ) : ?>
                    ...
                    <td id="lead-status-<?php echo esc_html( $userInfo->ID ); ?>">
                      <?php $status = apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/action/statuses', [], $userInfo->meta->status ); ?>
                      <?php $status = is_string( $status ) ? $status : ''; ?>
                      <span class="badge bg-soft-success p-2 text-success"><?php echo esc_html( $status ); ?></span>
                      <?php // echo apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/widgets/statustab', '', $userInfo ); ?>
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        <div class="media-support-info">
                          ...
                          <a class="mb-0" href="<?php echo esc_url( $userInfo->meta->meeting_link ); ?>" target="_blank">@<?php esc_html_e( 'Meeting link', 'advanced-gpt3-cv-builder' ); ?></a>
                          <?php endif; ?>
                        </div>
                      </div>
                    </td>
                    ...
                    <td>
                      <div class="d-flex justify-content-evenly">
                        <a class="btn btn-primary btn-icon btn-sm rounded-pill" href="<?php echo esc_url( apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/user/dashboardpermalink', $userInfo->ID, $userInfo->data->user_nicename ) ); ?>" role="button" target="_blank">
