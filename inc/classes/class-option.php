...
class Option {
  ...
  private $file;
	...
	protected function setup_hooks() {
		$this->file = ADVANCEDGPT3CVBUILDER_PROJECT__FILE__;
		$this->plugin_slug = 'advanced-gpt3-cv-builder';
		$this->plugin_name = 'advanced-gpt3-cv-builder';
		$this->textdomain = 'advanced-gpt3-cv-builder'; // str_replace('_', '-', $plugin_slug);
	...
	public function init() {
		$this->general = (object) wp_parse_args( apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/settings/general', [] ), [
			'page_title'					=> __( 'Configuration.', 'advanced-gpt3-cv-builder' ),
			'menu_title'					=> __( 'Config', 'advanced-gpt3-cv-builder' ),
			'role'								=> 'manage_options',
			'slug'								=> $this->plugin_slug,
			'page_header'					=> __( 'customization page.', 'advanced-gpt3-cv-builder' ),
			'page_subheader'			=> __( 'Your setting panel from where you can control and customize.', 'advanced-gpt3-cv-builder' ),
			'no_password'					=> __( 'A password is required.', 'advanced-gpt3-cv-builder' ),
		] );
	...
	public function add_settings_link( $links ) {
		$settings_link = '<a href="options-general.php?page='.$this->general->slug.'">' . __( 'Settings', 'advanced-gpt3-cv-builder' ) . '</a>';
		// array_push( $links, $settings_link );
		$links[] = $settings_link;
		$links[] = '<a href="https://www.fiverr.com/mahmud_remal/" target="_blank">' . __( 'Developer Support', 'advanced-gpt3-cv-builder' ) . '</a>';
		return $links;
	}
	...
	private function settings_fields() {
		$settings = [];
		$settings = apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/settings/fields', $settings );
		return $settings;
	}
	...
	public function display_field( $args ) {
		...
		switch( $field['type'] ) {
			...
			case 'radio':
			case 'select_multi':
				$html .= apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/settings/fields/label', '<br/><span class="description">' . $field['description'] . '</span>', $field );
			break;

			default:
				$html .= apply_filters( 'futurewordpress/project/advancedgpt3cvbuilder/settings/fields/label', '<label for="' . esc_attr( $field['id'] ) . '"><span class="description">' . $field['description'] . '</span></label>' . "\n", $field );
			break;
		}
